# Personas v1 Implementation Plan
...
- License MIT. Extension identifier `personas` (name is a placeholder; final marketplace name chosen at publish time).
...
├── media/personas.svg                      # activity bar icon
├── src/
...
**Files:**
- Create: `package.json`, `tsconfig.json`, `esbuild.mjs`, `vitest.config.ts`, `.gitignore`, `.vscodeignore`, `LICENSE`, `CHANGELOG.md`, `media/personas.svg`, `src/extension.ts`, `test/unit/smoke.test.ts`

**Interfaces:**
- Consumes: nothing (first task).
- Produces: `npm run build` (both bundles), `npm test` (vitest), `npm run lint` (tsc --noEmit). `src/extension.ts` exporting `activate`/`deactivate`. Command id `personas.showMatrix`.
...
```json
{
  ...
  "contributes": {
    "commands": [
      { "command": "personas.showMatrix", "title": "Personas: Show Extension Matrix", "icon": "$(extensions)" }
    ],
    "viewsContainers": {
      "activitybar": [{ "id": "personas", "title": "Personas", "icon": "media/personas.svg" }]
    },
    "views": { "personas": [{ "id": "personas.welcome", "name": "Profiles" }] },
    "viewsWelcome": [
      {
        "view": "personas.welcome",
        "contents": "Manage extensions across all your VS Code profiles.\n[Open Extension Matrix](command:personas.showMatrix)"
      }
    ]
  },
  "scripts": {
...
- [ ] **Step 5: Create `.gitignore`, `.vscodeignore`, `LICENSE`, `CHANGELOG.md`, `media/personas.svg`**
...
`media/personas.svg` (simple grid glyph, `currentColor` so it themes):
```svg
...
export function activate(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('personas.showMatrix', () => {
      void vscode.window.showInformationMessage('Personas: matrix coming soon.');
    }),
  );
}
...
Open the folder in VS Code, press F5 (creates `.vscode/launch.json` if prompted — accept the default "Run Extension" config and commit it). In the dev host, run "Personas: Show Extension Matrix" → info toast appears.
...
```ts
context.subscriptions.push(
  vscode.commands.registerCommand('personas.spikeToggle', async () => {
    try {
...
Also register it in `package.json` `contributes.commands` temporarily: `{ "command": "personas.spikeToggle", "title": "Personas: SPIKE toggle all-profiles" }`.
...
```bash
code --user-data-dir "$TMPDIR/personas-spike-data" --extensions-dir "$TMPDIR/personas-spike-ext" --new-window
```
(PowerShell: use `$env:TEMP\personas-spike-data` etc.)
...
```
Expected: lists the sandbox profile's extension. Without the two dir args, it must list your real default profile instead — confirming Personas must always pass both args explicitly (it derives both from its own runtime paths, so this also covers true portable-mode installs).
...
export class MatrixPanel {
  ...
  static show(
    ...
    const panel = vscode.window.createWebviewPanel('personasMatrix', 'Extension Matrix', vscode.ViewColumn.One, {
      enableScripts: true,
...
export function activate(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('personas.showMatrix', async () => {
      ...
      if ('error' in setup) {
        void vscode.window.showErrorMessage(`Personas can't manage profiles in this environment: ${setup.error}`);
        return;
      }
...
F5 → run "Personas: Show Extension Matrix" → panel opens showing "Loading…" (client comes next task). Set a breakpoint in `refresh()` and confirm `lastInventory.profiles` matches your real profiles and `warnings` is empty.

- [ ] **Step 6: Manual mutation check (real CLI, real profile)**

In the Debug Console: `await vscode.commands.executeCommand('personas.showMatrix')`, then temporarily add to `activate` (or evaluate via debug REPL) a call: install `ms-vscode.hexeditor` into a named test profile through the service, verify it appears in that profile's Extensions view, then uninstall. This validates `createNodeCliRunner` quoting on Windows (Task 8 Step 5). Revert any temporary code.
...
7. Installing an extension via the normal Extensions view (not Personas) refreshes the matrix within ~a second.
...
```json
{
  "name": "personas-hello-fixture",
  "displayName": "Personas Hello Fixture",
  "description": "Inert fixture extension used by Personas integration tests.",
  "version": "0.0.1",
  "publisher": "personas-tests",
  "license": "MIT",
  "engines": { "vscode": "^1.90.0" },
  "categories": ["Other"],
  "contributes": {}
...
`README.md`: one line (`Fixture for Personas integration tests.`). `LICENSE`: MIT text. (vsce requires all three.)
...
async function main(): Promise<void> {
  ...
  await runTests({
    ...
    extensionTestsEnv: { PERSONAS_IT_USER_DATA: userDataDir, PERSONAS_IT_EXT_DIR: extensionsDir },
  });
}
...
describe('Personas end-to-end against a sandboxed VS Code', () => {
  let vsixPath: string;
...
> "Personas" is a working name; the final marketplace name may differ.
...
Personas reads the same files VS Code itself maintains (the profile registry and per-profile extension lists) and performs installs/uninstalls through the official `code` command-line interface. It never writes to VS Code's internal state files.
...
- Remote workspaces (SSH/WSL/Containers/web) are not supported — Personas manages the local install it runs in.
...
```json
"repository": { "type": "git", "url": "https://github.com/SamErde/personas" }
```
...
Expected: all green; a `personas-0.1.0.vsix` file is produced. Install it into your real VS Code (`code --install-extension personas-0.1.0.vsix`) and run through the Task 11 Step 9 checklist once against your actual profiles.
